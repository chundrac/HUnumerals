...
feats_0,feats_10,feats=genfeats12gram()
...
#                      if (i,0) not in cache[-10:] or t > 10:
                        p = log(mu[w][m]) - log(len(set(morphs_10+morphs_0+[w]))) - log(i_curr.values().count(i)+1) - log(j_curr.values().count(0)+1)
...
#                          if (i,j) not in cache[-10:] or t > 10:
                            p = log(mu[w][m]) - log(len(set(morphs_10+morphs_0+[w[m:],w[:m]]))) - log(i_curr.values().count(i)+1) - log(j_curr.values().count(j)+1)
                            for f in set(feats_10[w][m]):
                                for k in range(1,feats_10[w][m].count(f)+1):
                                    p += log(omega[10][i][feats.index(f)]+k-1)
                            for l in range(1,len(feats_10[w][m])+1):
...
f = open('hindi11to99mu2unibigramMDL2annealdisperseMAP.txt','w')
