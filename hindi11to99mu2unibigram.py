...
#                      if (i,0) not in cache[-10:] or t > 10:
                        p = log(mu[w][m])# - log(sum([len(s) for s in set(morphs_10+morphs_0+[w])]))
...
#                          if (i,j) not in cache[-10:] or t > 10:
                            p = log(mu[w][m])# - log(sum([len(s) for s in set(morphs_10+morphs_0+[w[m:],w[:m]])]))
                            for f in set(feats_10[w][m]):
                                for k in range(1,feats_10[w][m].count(f)+1):
                                    p += log(omega[10][i][feats.index(f)]+k-1)
                            for l in range(1,len(feats_10[w][m])+1):
                            ...
                            pis[(m,i,j)] = p
            pi_vals = np.array(pis.values())
            Z = sum(exp(pi_vals))
            for k in pis.keys():
                pis[k] = exp(pi_vals[pis.keys().index(k)])/Z
...
f = open('hindi11to99mu2unibigramMAP.txt','w')
